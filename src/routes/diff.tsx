          ...
          <div className="surface-panel p-5">
            ...
            {baseAnalysis && (
              <div className="mt-3 text-xs text-muted-foreground">
                ...
                <br />
                {baseAnalysis.nodes.length} nodes · {baseAnalysis.edges.length}{" "}
                edges
              </div>
            )}
          </div>
          ...
          <div className="surface-panel p-5">
            ...
            {headAnalysis && (
              <div className="mt-3 text-xs text-muted-foreground">
                ...
                <br />
                {headAnalysis.nodes.length} nodes · {headAnalysis.edges.length}{" "}
                edges
              </div>
            )}
          </div>
          ...
          <div className="mt-8 surface-panel p-5">
            ...
            <div className="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
              ...
              <StatCard
                ...
                value={diff.summary.nodesModified}
                color="warning"
              />
              <StatCard
                title="Edges Added"
                value={diff.summary.edgesAdded}
                color="success"
              />
              <StatCard
                title="Edges Deleted"
              ...
              <StatCard
                title="LOC Added"
                value={diff.summary.locAdded}
                color="success"
              />
              <StatCard
                title="LOC Removed"
                value={diff.summary.locRemoved}
                color="destructive"
              />
              <StatCard
                title="Complexity ↑"
                    ...
                    {diff.modifiedNodes.map(({ node, changes }) => (
                      ...
                      >
                        ...
                        <div className="mt-2 text-xs text-muted-foreground">
                          ...
                          {changes.complexity && (
                            <span className="ml-4">
                              Complexity: {changes.complexity.base} →{" "}
                              {changes.complexity.head}
    ...
    <div className="mt-3">
      ...
      >
        ...
        {entries.map((entry) => (
          ...
          >
            {entry.owner}/{entry.repo} @ {entry.branch} ({entry.nodeCount}n,{" "}
            {entry.edgeCount}e)
          </option>
        ))}
      </select>
      ...
      {edges.slice(0, 10).map((edge, index) => (
        ...
        >
          ...
          <span className="font-mono">
            {edge.source.split(":").pop() || edge.source}
          </span>
